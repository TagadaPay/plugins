...
export function ConfigProvider({ 
  ...
  // In development with URL params, prioritize local config over SDK config
  const urlParams = typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : null
  const hasConfigParam = urlParams?.has('config')
  
  const configState = (sdkConfig && !hasConfigParam) ? {
  ...
  useEffect(() => {
    if (!import.meta.env.PROD) {
      if (hasConfigParam) {
        console.log('[ConfigProvider] Using local config due to URL param:', configState.config)
      } else if (sdkConfig) {
        console.log('[ConfigProvider] Using SDK config:', configState.config)
      } else {
        console.log('[ConfigProvider] Using local config (no SDK):', configState.config)
      }
    }
  }, [sdkConfig, configState.config, hasConfigParam])
