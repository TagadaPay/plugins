...
export function DevConfigSwitcher() {
  const { configName, reloadConfig } = useConfigContext()
  ...
  // Get available config names from /config/ directory
  useEffect(() => {
    const configs = ['default', 'production-store', 'showcase-customization'] // Available in /config/ directory
    setAvailableConfigs(configs)
  }, [])

  const switchToConfig = (newConfigName: string) => {
    if (newConfigName !== configName) {
      console.log('[DevConfigSwitcher] Switching from', configName, 'to', newConfigName)
      
      // Simple approach: just reload the page with the new config parameter
      const url = new URL(window.location.href)
      url.searchParams.set('config', newConfigName)
      window.location.href = url.toString()
    } else {
      console.log('[DevConfigSwitcher] Same config selected, no change needed')
      setIsOpen(false)
    }
  }
