...
export const ProductCard = ({
  ...
  const matchingPromotion = promotions.find((promo: any) => {
    const promoName =
      typeof promo.name === 'string' ? promo.name : promo.name?.en || '';
  ...
  return (
    ...
    >
      ...
      {badge && (
        <div
          className={`absolute top-3 right-3 z-20 px-2 py-1 rounded-full text-xs font-bold shadow-lg
          ${
            badge === 'BEST VALUE'
      ...
      <CardContent className="flex flex-row md:items-center items-start flex-1 p-4 gap-3 md:gap-4">
        ...
        {image && (
          <div className="flex-shrink-0 flex items-center justify-center w-24 h-24 md:w-32 md:h-32">
            <img
              ...
              alt={`${
                uiText?.productAltTextPrefix || 'Pure Shilajit Gummies -'
              } ${title}`}
              width={120}
              height={120}
        ...
        <div className="flex flex-col flex-1 items-start justify-center">
          {/* Title */}
          <h3 className="text-lg md:text-xl font-extrabold text-gray-900 tracking-tight mb-1">
            {title}
          </h3>
          ...
          {matchingPromotion?.freeGift && (
            <div className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 mb-3 shadow-sm bg-primary-color text-white">
              <span className="text-xs font-bold uppercase tracking-wide">
                {typeof matchingPromotion.freeGift === 'string'
                  ? matchingPromotion.freeGift
                  : t(matchingPromotion.freeGift)}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="text-lg font-bold text-primary-color">
              {formatSimpleMoney(preview.totalAdjustedAmount, currency)}
            </span>
            <span className="text-xs text-gray-400 line-through">
              {formatSimpleMoney(preview.totalAmount, currency)}
            </span>
            <span className="text-xs text-green-600 font-semibold ml-2">
              {t(uiText?.youSaveText) || 'You save $'}
