...
import { usePostPurchases, formatMoney, usePluginConfig } from '@tagadapay/plugin-sdk/react';
...
const PostPurchaseOffer: React.FC<PostPurchaseOfferProps> = ({ orderId }) => {
  ...
  const handleVariantSelect = useCallback(async (offerId: string, productId: string, variantId: string) => {
    try {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="text-center max-w-md mx-auto p-6">
          ...
          <p className="text-gray-600 mb-4">We're having trouble loading your special offers right now.</p>
  ...
  const discountPercentage = Math.round((1 - summary.totalAdjustedAmount / summary.totalAmount) * 100);
            ...
            <div className="flex justify-center items-center gap-3 mb-6">
              {offers.map((_, index) => (
                <div
                  ...
                  style={{
                    backgroundColor: index === currentOfferIndex ? primaryColor : undefined
                  }}
                />
              ))}
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-6 border border-gray-100">
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 sm:gap-6">
              ...
              <div className="text-center lg:text-right">
                <div className="bg-white/20 backdrop-blur-sm rounded-xl px-3 sm:px-4 py-2 sm:py-3 inline-block border border-white/30">
                  <div className="text-xl sm:text-2xl lg:text-3xl font-bold">{discountPercentage}%</div>
          ...
          <div className="p-4 sm:p-6">
            <h3 className="text-lg sm:text-xl font-bold mb-4 flex items-center text-gray-900">
              ...
              What you'll get ({summary.items.length} {summary.items.length === 1 ? 'item' : 'items'})
            </h3>

            <div className={`grid gap-4 ${summary.items.length > 1 ? 'lg:grid-cols-2' : ''}`}>
              {summary.items.map((item: any) => {
                const availableVariants = getAvailableVariants(currentOffer.id, item.productId);
                const isLoadingThisVariant = isLoadingVariants(currentOffer.id, item.productId);
                ...
                return (
                  ...
                  >
                    <div className={`flex flex-col gap-3 ${summary.items.length === 1 ? 'sm:flex-row sm:gap-6' : 'lg:flex-row lg:gap-4'}`}>
                      {/* Product Image */}
                      <div className={`relative bg-white rounded-lg overflow-hidden flex-shrink-0 shadow-sm ${
                      ...
                      <div className="flex-1 space-y-2">
                        <div>
                          <h4 className="text-base sm:text-lg font-bold text-gray-900 mb-1">{item.product.name}</h4>
                        ...
                        {availableVariants.length > 1 ? (
                          <div>
                            <p className="text-sm font-semibold text-gray-800 mb-2">Choose variant:</p>
                            <div className="space-y-2">
                              {availableVariants.map((variant: any) => {
                                const isSelected = item.variantId === variant.variantId;
                                ...
                                const variantPrice = (() => {
                                  ...
                                  if (typeof variant.currencyOptions === 'object' && summary.currency in variant.currencyOptions) {
                                    const currencyData = (variant.currencyOptions as any)[summary.currency];
                                    return currencyData?.amount || 0;
                                  }
                                  
                                  // Handle direct amount structure
                                  if ('amount' in variant.currencyOptions) {
                                    return (variant.currencyOptions as any).amount || 0;
                                  }
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={() => handleVariantSelect(currentOffer.id, item.productId, variant.variantId)}
                                    disabled={isLoadingThisVariant || isOfferUpdating}
                                    className={`
                                    ...
                                    style={{
                                      ...
                                      '--tw-ring-color': isSelected ? `${primaryColor}40` : undefined,
                                  ...
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="text-right">
                                        <p className={`font-bold text-base sm:text-lg ${isSelected ? 'text-blue-600' : 'text-gray-700'}`}>
                                          {formatMoney(variantPrice, summary.currency)}
                                        </p>
                                      </div>
                                    </div>
                                  </button>
          ...
          <div className="p-6 sm:p-8 bg-gradient-to-r from-gray-50 to-gray-100 border-t">
            <button
              ...
              disabled={isProcessing || isOfferUpdating || summary.items.some((item: any) => isLoadingVariants(currentOffer.id, item.productId))}
              ...
              style={{
                background: isProcessing || isOfferUpdating ? '#9ca3af' : `linear-gradient(to right, ${primaryColor}, ${accentColor})`,
                filter: isProcessing || isOfferUpdating ? 'none' : 'brightness(1) hover:brightness(0.9)'
              }}
            >
              {isProcessing ? (
                <>
                ...
                <>
                  ...
                  <span className="font-bold">
                    Add to Cart - {formatMoney(summary.totalAdjustedAmount, summary.currency)}
                  </span>
                </>
              )}
            </button>
