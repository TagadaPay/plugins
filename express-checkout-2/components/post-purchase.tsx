...
export default function PostPurchase() {
  ...
  const postPurchaseTexts = texts?.postPurchase;
  ...
  const handleAcceptOffer = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        String(
          t(postPurchaseTexts?.toastOfferAccepted, "Offer added to your order!")
        )
      );
          ...
          <Card className="w-full">
            <CardHeader>
              <CardTitle className="text-red-600">
                {t(postPurchaseTexts?.errorTitle, "Unable to load offers")}
              </CardTitle>
            </CardHeader>
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto flex max-w-3xl items-center justify-center px-4 py-16">
          <div className="text-center">
            <div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-b-2 border-gray-900" />
            <p className="text-gray-700">
              {t(postPurchaseTexts?.preparingTitle, "Preparing your offer...")}
            </p>
            <p className="mt-1 text-xs text-gray-500">
              {t(
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mb-8 space-y-4 text-center">
          <div className="inline-flex items-center gap-3 rounded-full border border-gray-200 bg-white px-4 py-2 text-xs font-medium text-gray-700 shadow-sm">
            <Clock className="h-4 w-4" />
            <span className="font-mono text-sm font-semibold">
              {formatTime(timeLeft)}
            </span>
            <span className="hidden sm:inline">
              {t(postPurchaseTexts?.timerLabel, "left to claim this offer")}
            </span>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
          ...
          <Card className="overflow-hidden border border-gray-200">
            <div className="border-b border-gray-200 bg-[var(--checkout-badge-bg)] px-4 py-3 text-left text-sm font-medium text-white sm:px-6">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="text-xs uppercase tracking-wide opacity-80">
                    {t(postPurchaseTexts?.bannerBadge, "Limited‑time upgrade")}
                  </div>
              ...
              <div className="space-y-3">
                {summary.items.map((item: any) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className="flex flex-col gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:flex-row sm:items-center sm:p-4"
                    >
                      <div className="flex-shrink-0">
                        <div className="relative h-20 w-20 overflow-hidden rounded-md bg-white shadow-sm">
                          ...
                          <div className="absolute right-1 top-1 rounded-full bg-[var(--checkout-badge-bg)] px-2 py-0.5 text-xs font-semibold text-white shadow">
                            ×{item.quantity}
                          </div>
                        </div>
                      </div>
                              ...
                              {availableVariants.map((variant: any) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`flex w-full items-center justify-between rounded-md border px-3 py-2 text-left text-xs sm:text-sm ${
                                      isSelected
          ...
          {/* Summary / actions */}
          <Card className="h-fit border border-gray-200 bg-white">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base text-gray-900">
                <Package className="h-4 w-4" />
                {t(postPurchaseTexts?.summaryTitle, "Order upgrade summary")}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2 text-sm">
                ...
                {summary.totalAmount > summary.totalAdjustedAmount && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">
                      {t(postPurchaseTexts?.summaryYouSaveLabel, "You save")}
                    </span>
